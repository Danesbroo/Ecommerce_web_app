...
function CheckoutForm() {
    ...
    const placeOrder = async (e) => {
        ...
        try {
            const result = await axios.post(`${process.env.NEXT_PUBLIC_BASE_URL}/api/website/orders/order-placed`,
                {
                    net_amount: finalTotal,
                    productInfo: cartItems.map(item => ({
                        product_id: item._id,
    ...
    return (
        <>
            <form onSubmit={placeOrder} className='px-3 md:px-20 py-10'>
                ...
                <div className='flex items-center gap-5 my-7'>
                    ...
                    <div>
                        <span onClick={OtherShipping} className=' cursor-pointer bg-black font-bold rounded px-4 py-2 text-white text-[14px] md:text-xl'>Ship To A Different Address</span>
                    </div>
                </div>
